...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-Process -Path "$dirFiles\TableauReader-64bit-2021-2-0.exe" -Parameters "ACCEPTEULA=1 AUTOUPDATE=0 DATABASEDRIVERS=1 DESKTOPSHORTCUT=0 /quiet /norestart" -WindowStyle "Hidden" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		##Remove-MSIApplications -Name 'Tableau Reader'
		$exitCode = Execute-Process -Path "$dirFiles\TableauReader-64bit-2021-2-0.exe" -Parameters "/uninstall /quiet /norestart" -WindowStyle "Hidden" -PassThru
        If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
